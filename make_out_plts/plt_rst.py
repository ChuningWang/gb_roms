import numpy as np
import matplotlib.pyplot as plt
import netCDF4 as nc

import subprocess
import pyroms
from pyroms_toolbox import rx0

grd = pyroms.grid.get_ROMS_grid('GB')
msku = grd.hgrid.mask_u
mskv = grd.hgrid.mask_v

var = 'u'
ex = 'abs'
rmap = 'full'
plth = -1

h = grd.vgrid.h
h = np.ma.masked_where(grd.hgrid.mask==0, h)
hraw = grd.vgrid.hraw.squeeze()
hraw = np.ma.masked_where(grd.hgrid.mask==0, hraw)

rx = rx0(h, grd.hgrid.mask)

fh = nc.Dataset('/Volumes/R1/scratch/chuning/gb_roms/outputs/UPWELLING_ICE_rst.nc')

# u_rst = fh.variables['u'][:].squeeze()
# v_rst = fh.variables['v'][:].squeeze()
# temp_rst = fh.variables['temp'][:, 0, :, :, :].squeeze()
# salt_rst = fh.variables['salt'][:, 0, :, :, :].squeeze()

data = fh.variables[var][:, 0, :, :, :].squeeze()
if var=='u':
    data = np.ma.masked_where(np.tile(msku==0, (40, 1, 1)), data)
elif var=='v':
    data = np.ma.masked_where(np.tile(mskv==0, (40, 1, 1)), data)

fh.close()

x = np.arange(h.shape[0])
y = np.arange(h.shape[1])

if ex=='abs':
    idx = np.where(np.abs(data) == np.abs(data).max())
elif ex=='max':
    idx = np.where(data == data.max())
elif ex=='min':
    idx = np.where(data == data.min())
zlevel = idx[0].data[0]
yy = idx[1].data[0]
xx = idx[2].data[0]

if rmap=='full':
    xmin = 0
    xmax = 1000
    ymin = 0
    ymax = 500
elif rmap=='zoom':
    xmin = xx-20
    xmax = xx+20
    ymin = yy-20
    ymax = yy+20

plt.figure()
plt.pcolor(y[ymin:ymax], x[xmin:xmax], data[zlevel, xmin:xmax, ymin:ymax].squeeze())
plt.plot([xmin, xmax], [yy, yy], '--k', lw=0.01)
plt.plot([xx, xx], [ymin, ymax], '--k', lw=0.01)
plt.xlim(ymin, ymax)
plt.ylim(xmin, xmax)
plt.colorbar()
plt.title(str(xx)+ ',' + str(yy) + ',' + str(zlevel))
plt.savefig('/Volumes/R1/scratch/chuning/gb_roms/figs/rst_' + var + '_' + ex + '.png', format='png', dpi=900)
plt.close()

if plth==1:
    plt.figure()
    plt.pcolor(y[ymin:ymax], x[xmin:xmax], h[xmin:xmax, ymin:ymax], cmap='OrRd')
    plt.plot([xmin, xmax], [yy, yy], '--k', lw=0.01)
    plt.plot([xx, xx], [ymin, ymax], '--k', lw=0.01)
    plt.xlim(ymin, ymax)
    plt.ylim(xmin, xmax)
    plt.clim(0, 500)
    plt.colorbar()
    plt.title('Depth [m]')
    plt.savefig('/Volumes/R1/scratch/chuning/gb_roms/figs/depth_' + var + '_' + ex + '.png', format='png', dpi=900)
    plt.close()

    plt.figure()
    plt.pcolor(y[ymin:ymax], x[xmin:xmax], hraw[xmin:xmax, ymin:ymax], cmap='OrRd')
    plt.plot([xmin, xmax], [yy, yy], '--k', lw=0.01)
    plt.plot([xx, xx], [ymin, ymax], '--k', lw=0.01)
    plt.xlim(ymin, ymax)
    plt.ylim(xmin, xmax)
    plt.clim(0, 500)
    plt.colorbar()
    plt.title('hraw [m]')
    plt.savefig('/Volumes/R1/scratch/chuning/gb_roms/figs/hraw_' + var + '_' + ex + '.png', format='png', dpi=900)
    plt.close()

    plt.figure()
    plt.pcolor(y[ymin:ymax], x[xmin:xmax], h[xmin:xmax, ymin:ymax]-hraw[xmin:xmax, ymin:ymax], cmap='OrRd')
    plt.plot([xmin, xmax], [yy, yy], '--k', lw=0.01)
    plt.plot([xx, xx], [ymin, ymax], '--k', lw=0.01)
    plt.xlim(ymin, ymax)
    plt.ylim(xmin, xmax)
    plt.colorbar()
    plt.title('hdiff [m]')
    plt.savefig('/Volumes/R1/scratch/chuning/gb_roms/figs/hdiff_' + var + '_' + ex + '.png', format='png', dpi=900)
    plt.close()

    plt.figure()
    plt.pcolor(y[ymin:ymax], x[xmin:xmax], rx[xmin:xmax, ymin:ymax], cmap='OrRd')
    plt.plot([xmin, xmax], [yy, yy], '--k', lw=0.01)
    plt.plot([xx, xx], [ymin, ymax], '--k', lw=0.01)
    plt.xlim(ymin, ymax)
    plt.ylim(xmin, xmax)
    plt.clim(0, 0.3)
    plt.colorbar()
    plt.title('rx0')
    plt.savefig('/Volumes/R1/scratch/chuning/gb_roms/figs/rx0_' + var + '_' + ex + '.png', format='png', dpi=900)
    plt.close()


scp = 0
if scp==1:
    subprocess.call('', shell=True)

