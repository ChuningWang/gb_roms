...
timeraw = fh.variables['t'][:]
t1 = (t_ini-t_base).days
t2 = (t_end-t_base).days
mskt = (timeraw >= t1) & (timeraw <= t2)
time = timeraw[mskt]
pytime = nc.num2date(time, 'days since 1900-01-01')
yearday = np.array([pyt.timetuple().tm_yday for pyt in pytime])

# calculate climatology
data = np.zeros((len(yearday), eta, xi))
pytimeraw = nc.num2date(timeraw, 'days since 1900-01-01')
yeardayraw = np.array([pyt.timetuple().tm_yday for pyt in pytimeraw])

for i, yd in enumerate(yearday):
    mskt = yeardayraw == yd
    data[i, :, :] = fh.variables['discharge'][mskt, :, :].mean(axis=0)

# dataraw = fh.variables['discharge'][:, :, :]
fh.close()
...
temp = np.concatenate((temp, temp, temp))
ctd_time = np.concatenate((ctd_time-365, ctd_time, ctd_time+365))

# smooth it a bit
temp[1:-1] = 1./3.*(temp[2:]+temp[:-2]+temp[1:-1])
temp[0] = 1./3.*(temp[-1]+temp[0]+temp[1])
temp[-1] = 1./3.*(temp[-2]+temp[-1]+temp[0])
...
spl = splrep(ctd_time, temp)
river_temp = splev(river_yearday, spl)
